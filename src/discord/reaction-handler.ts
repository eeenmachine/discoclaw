...
export function createReactionAddHandler(
  ...
  return async (reaction, user) => {
    try {
      ...
      const addPrompt = reactionPromptText('add');
        ...
        try {
          ...
          let prompt =
            ...
            (durableSection
              ...
              : '') +
            `\n\n---\nReaction event:\n` +
            addPrompt.eventLine(reactingUser, user.id, emoji, channelLabel) + `\n\n` +
...
export function createReactionRemoveHandler(
  ...
  return async (reaction, user) => {
    try {
      const metrics = params.metrics ?? globalMetrics;
      metrics.increment('discord.reaction_remove.received');
      ...
      try {
        ...
        params.log?.warn({ err }, 'reaction-remove:partial fetch failed (reaction)');
        return;
      }
      try {
        if (reaction.message.partial) await reaction.message.fetch();
      } catch (err) {
        params.log?.warn({ err }, 'reaction-remove:partial fetch failed (message)');
        return;
      }
      ...
      const removePrompt = reactionPromptText('remove');
        ...
        try {
          ...
          if (params.autoJoinThreads && isThread) {
            ...
            if (joinable && !joined && typeof ch?.join === 'function') {
              try {
                await ch.join();
                params.log?.info({ threadId: String(ch.id ?? ''), parentId: String(ch.parentId ?? '') }, 'reaction-remove:thread joined');
              } catch (err) {
                params.log?.warn({ err, threadId: String(ch?.id ?? '') }, 'reaction-remove:thread failed to join');
              }
            }
          }
          ...
          if (params.discordChannelContext && params.autoIndexChannelContext) {
            ...
            try {
              ...
              params.log?.error({ err, channelId: id }, 'reaction-remove:context failed to ensure channel context');
            }
          }
          ...
          if (params.requireChannelContext && !channelCtx.contextPath) {
            params.log?.warn({ channelId: channelCtx.channelId }, 'reaction-remove:missing required channel context');
            return;
          }
          ...
          let prompt =
            ...
            (durableSection
              ...
              : '') +
            `\n\n---\nReaction event:\n` +
            removePrompt.eventLine(reactingUser, user.id, emoji, channelLabel) + `\n\n` +
            `Original message by ${messageAuthor} (ID: ${messageAuthorId}):\n` +
            messageContent;
          ...
          for await (const evt of params.runtime.invoke({
            ...
            if (evt.type === 'text_final') {
              ...
              invokeError = evt.message;
              metrics.recordInvokeResult('reaction', Date.now() - t0, false, evt.message);
              params.log?.error({ sessionKey, error: evt.message }, 'reaction-remove:runtime error');
              params.log?.warn({ flow: 'reaction', sessionKey, error: evt.message }, 'obs.invoke.error');
              // eslint-disable-next-line @typescript-eslint/no-floating-promises
              statusRef?.current?.runtimeError({ sessionKey, channelName: channelCtx.channelName }, evt.message);
              await replyThenSendChunks(reaction.message as any, mapRuntimeErrorToUserMessage(evt.message));
          ...
          params.log?.info({ sessionKey, sessionId, ms: Date.now() - t0, hadError: Boolean(invokeError) }, 'reaction-remove:invoke:end');
          ...
          metrics.increment('discord.reaction_remove.handler_error');
          params.log?.error({ err, sessionKey }, 'reaction-remove:handler failed');
          // eslint-disable-next-line @typescript-eslint/no-floating-promises
          statusRef?.current?.handlerError({ sessionKey }, err);
        }
      });
      ...
      const metrics = params.metrics ?? globalMetrics;
      metrics.increment('discord.reaction_remove.handler_wrapper_error');
      params.log?.error({ err }, 'reaction-remove:messageReactionRemove failed');
    }
  };
}
